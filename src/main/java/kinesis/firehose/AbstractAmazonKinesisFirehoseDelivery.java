...
public abstract class AbstractAmazonKinesisFirehoseDelivery {
	...
	private static final String IAM_ROLE_PERMISSIONS_POLICY_WITH_KMS_RESOURCES_DOCUMENT = "firehose/permissionsPolicyDocument.json";
	// IAM Permissions Policy Document without KMS Resources.
	private static final String IAM_ROLE_PERMISSIONS_POLICY_WITHOUT_KMS_RESOURCES_DOCUMENT = "firehose/permissionsPolicyWithoutKMSResourcesDocument.json";
	// IAM Trust Policy Document
	private static final String IAM_ROLE_TRUST_POLICY_DOCUMENT = "firehose/trustPolicyDocument.json";
	...
	private static final String PUT_RECORD_STREAM_SOURCE = "firehose/putRecordInput.txt";
	private static final String BATCH_PUT_STREAM_SOURCE = "firehose/batchPutInput.txt";
	...
	protected static List<String> listDeliveryStreams() {
		...
		ListDeliveryStreamsResult listDeliveryStreamsResult = firehoseClient.listDeliveryStreams(listDeliveryStreamsRequest);
	...
	protected static void putRecordBatchIntoDeliveryStream() throws IOException {
		try (InputStream inputStream = Thread.currentThread().getContextClassLoader()
			.getResourceAsStream(BATCH_PUT_STREAM_SOURCE)) {
			if (inputStream == null) {
	...
	protected static String createIamRole(String s3Prefix) throws InterruptedException {
		try {
			...
			iamClient
				.createRole(new CreateRoleRequest().withRoleName(iamRoleName).withAssumeRolePolicyDocument(getTrustPolicy()));
			...
			LOG.error(
				String.format("Please check the trust policy document for malformation: %s", IAM_ROLE_TRUST_POLICY_DOCUMENT));
	...
	protected static void waitForDataDelivery(int waitTimeSecs) throws InterruptedException {
		LOG.info(
			"Since the Buffering Hints IntervalInSeconds parameter is specified as: " + waitTimeSecs + " seconds. Waiting for "
				+ waitTimeSecs + " seconds for the data to be written to S3 bucket");
	...
	protected static void putRolePolicy(String s3Prefix) {
		try {
			...
			String permissionsPolicyDocument = containsKMSKeyARN() ?
				getPermissionsPolicyWithKMSResources(s3Prefix) :
				getPermissionsPolicyWithoutKMSResources();
			iamClient.putRolePolicy(new PutRolePolicyRequest().withRoleName(iamRoleName).withPolicyName(FIREHOSE_ROLE_POLICY_NAME)
				.withPolicyDocument(permissionsPolicyDocument));
		} catch (MalformedPolicyDocumentException policyDocumentException) {
			LOG.error(String.format("Please check the permissions policy document for malformation: %s", containsKMSKeyARN() ?
	...
	private static String getTrustPolicy() {
		return readResource(IAM_ROLE_TRUST_POLICY_DOCUMENT).replace("{{CUSTOMER_ACCOUNT_ID}}", accountId);
	}
	...
	private static String getPermissionsPolicyWithKMSResources(String s3Prefix) {
		return readResource(IAM_ROLE_PERMISSIONS_POLICY_WITH_KMS_RESOURCES_DOCUMENT).replace("{{S3_BUCKET_NAME}}", s3BucketName)
			.replace("{{KMS_KEY_ARN}}", s3DestinationAWSKMSKeyId).replace("{{S3_REGION}}", s3RegionName)
