...
public class AmazonKinesisFirehoseToS3Sample extends AbstractAmazonKinesisFirehoseDelivery {
	...
	// Properties File
	private static final String CONFIG_FILE = "firehose/firehosetos3sample.properties";
	// Logger
	...
	private static void loadConfig() throws FileNotFoundException, IOException {
		...
		updateSizeInMBs = StringUtils.isNullOrEmpty(updateSizeInMBsProperty) ?
			null :
		...
		updateIntervalInSeconds = StringUtils.isNullOrEmpty(updateIntervalInSecondsProperty) ?
			null :
		...
		if (!deliveryStreamExists) {
			...
			if (!StringUtils.isNullOrEmpty(s3DestinationAWSKMSKeyId)) {
				encryptionConfiguration
					.setKMSEncryptionConfig(new KMSEncryptionConfig().withAWSKMSKeyARN(s3DestinationAWSKMSKeyId));
			} else {
