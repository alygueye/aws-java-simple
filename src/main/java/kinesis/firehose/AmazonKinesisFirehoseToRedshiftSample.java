...
public class AmazonKinesisFirehoseToRedshiftSample extends AbstractAmazonKinesisFirehoseDelivery {
	...
	// Properties file
	private static final String CONFIG_FILE = "firehose/firehosetoredshiftsample.properties";
	// Logger
	...
	private static void init() throws Exception {
		...
		if (StringUtils.isNullOrEmpty(accountId)) {
			throw new IllegalArgumentException("AccountId is empty. Please enter the accountId in " + CONFIG_FILE + " file");
		}
	}
	...
	private static void loadConfig() throws FileNotFoundException, IOException {
		...
		s3DestinationIntervalInSeconds = StringUtils.isNullOrEmpty(intervalInSecondsProperty) ?
			null :
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			int waitTimeSecs = s3DestinationIntervalInSeconds == null ?
				DEFAULT_WAIT_INTERVAL_FOR_DATA_DELIVERY_SECS :
	...
	private static void createDeliveryStream() throws Exception {
		...
		if (!deliveryStreamExists) {
			...
			redshiftDestinationConfiguration.withClusterJDBCURL(clusterJDBCUrl).withRoleARN(iamRoleArn).withUsername(username)
				.withPassword(password).withCopyCommand(copyCommand).withS3Configuration(redshiftS3Configuration);
	...
	private static void updateDeliveryStream() {
		...
		CopyCommand updatedCopyCommand = new CopyCommand().withDataTableName(dataTableName).withCopyOptions(updatedCopyOptions);
		RedshiftDestinationUpdate redshiftDestinationUpdate = new RedshiftDestinationUpdate().withCopyCommand(updatedCopyCommand);
